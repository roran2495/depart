...
namespace ClearSky
{
    public class PlayerController : MonoBehaviour
    {
        ...
        public Slider healthSlider; // 引用到 Slider 组件
        public TMP_Text bloodValue;
        private int maxHealth = 100; // 最大生命值
        private int currentHealth; // 当前生命值
        public AudioSource attackSource; //发出攻击的音效
        public AudioSource jumpSource; //跳跃的音效
        public AudioSource deathSource; //死亡的音效
        public AudioSource winSource; //胜利的音效
        ...
        void Start()
        {
            ...
            bloodValue.text = "Blood : " + currentHealth + " / " + maxHealth;
        }
        ...
        private void OnTriggerStay2D(Collider2D other)
        {
            ...
            if (other.tag == "Die")
            {
                ...
                bloodValue.text = "Blood : " + currentHealth + " / " + maxHealth;
                deathSource.Play();
                gameController.ToggleCanva(2);
            }
            if (other.tag == "Finish")
            {
                winSource.Play();
                gameController.ToggleCanva(3);
            }
        }

        private void OnCollisionEnter2D(Collision2D collision)
        {
            // 判断碰撞对象是否是 "Bark" 标签的游戏对象
            if (collision.gameObject.CompareTag("Bark"))
            {
                anim.SetBool("isJump", false);
            }
            // 判断碰撞对象是否是 "Monster" 标签的游戏对象
            ...
            if (collision.gameObject.CompareTag("MonsterBall"))
            {
